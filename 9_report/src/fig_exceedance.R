fig_exceedance <- function(fig_ind, config_fig_yml, preds_ind, agg_nwis_ind, remake_file, config_file) {
  ...
  # error
  cumulative_freq <- left_join(agg_nwis$flux, preds_df,
  ...
  # create the plot
  g <- ggplot(cumulative_freq, aes(x = daily_mean_flux/1000, y = freq)) +
    geom_point() +
